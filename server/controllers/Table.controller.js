...
const createTable= async(req,res)=>{
    ...
    try{
        const tablecreated= await TableModel.create({tablenum,description, chairs, isValid});
...
const showAllTables =async(_req,res)=>{
    try{
        const allTable =await TableModel.find();
                return res.status(200).json({"tables" : allTable})
            }
    catch (error) {
...
const showOneTable = async(req,res)=>{
    ...
    try{
        ...
        else    return     res.status(200).json({"table data ":oneTable
        })
        }
        catch(e){
    ...
    const updateTable = async(req, res)=>{
        let id =parseInt(req.params["tableid"])
        const tablenum = req.body.tablenum;
        const description = req.body.description;
        const chairs = req.body.chairs;
        const isValid = req.body.isValid;
        try{
            const updatedTable= await TableModel.findByIdAndUpdate({_id:id},{$set:{
                tablenum:tablenum,
                ...
                if(!updatedTable ){
                    ...
                    else{ return   res.status(201).json({'Updated Data':updatedTable });
                    }
        }catch(err){
